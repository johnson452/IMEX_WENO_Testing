...
app.grid.n_t(app.grid.NT-1) = sum(app.n);
app.grid.nu_t(app.grid.NT-1) = sum((app.n).*(app.u));
app.grid.nT_t(app.grid.NT-1) = sum((app.n).*(app.T));
time = linspace(app.grid.t_min,app.grid.time,app.grid.NT-1);
...
subplot(2,3,6)
plot(time,app.grid.n_t(1:app.grid.NT-1));
hold on
plot(time,app.grid.nu_t(1:app.grid.NT-1));
hold on
plot(time,app.grid.nT_t(1:app.grid.NT-1));
title("M(t)")
xlabel("t")
ylabel("M(t)")
legend("n_{tot}(t)","nu_{tot}(t)","nT_{tot}(t)")
